        ...
        {data.slice(0, 50).map(row => (
          <div key={row.id} className="flex gap-3 text-xs font-mono py-1 border-b border-gray-50">
            <span className="text-gray-300 flex-shrink-0">
              {new Date(row.detected_at).toLocaleTimeString('id-ID')}
            </span>
            <span className="text-blue-500 flex-shrink-0 w-28 truncate">{row.ip_address}</span>
            <span className="text-gray-400 flex-shrink-0 w-24 truncate">{row.uri}</span>
            <span className="text-gray-500 flex-1">{row.is_tool ? 'Tool terdeteksi' : 'Manual scan'}</span>
            <span className="text-amber-400 flex-shrink-0">+{row.score_delta}</span>
          </div>
        ))}
        {data.length === 0 && (
          <p className="text-center text-gray-400 text-xs py-4">Belum ada aktivitas</p>
        )}
      </div>
    </div>
  )
